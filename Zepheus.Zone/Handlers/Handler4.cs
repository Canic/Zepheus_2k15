    ...
    public sealed class Handler4
    {
        ...
        public static void SendEquippedList(ZoneCharacter character)
        {
            using (var packet = new Packet(SH4Type.CharacterItemList))
            {
                ...
                packet.WriteByte(0xAB);         // UNK    (In newest client it exists, in bit older, not) // might be shit from old buffers lol
        ...
        {
            using(var packet = new Packet(SH4Type.CharacterTimedItemList)) {
                //ushort count = 0;
