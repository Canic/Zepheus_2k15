...
namespace Zepheus.Zone.Handlers
{
    public sealed class Handler8
    {
        ...
        [PacketHandler(CH8Type.LoginGMAuthentication)]
        public static void LoginGMAuthenticate(ZoneClient client, Packet packet)
        {
            ZoneCharacter character = client.Character;
            byte len;
            string command;

            if (!packet.TryReadByte(out len) || !packet.TryReadString(out command, len))
            {
                return;
            }
        }
        ...
        private static void SendSellHPSP(ZoneCharacter character)
        {
            using (var packet = new Packet(SH15Type.SellHPSPStones))
            {
                packet.WriteInt(648); //unk
                packet.WriteInt(DataProvider.Instance.GetBaseStats(character.Job, character.Level).MAXSoulHP); //max HP Stones
                packet.WriteInt(DataProvider.Instance.GetBaseStats(character.Job, character.Level).PriceHPStone); //sell Price
                packet.WriteInt(695); //unk
                packet.WriteInt(DataProvider.Instance.GetBaseStats(character.Job, character.Level).MAXSoulSP); //max SP Stones
                packet.WriteInt(DataProvider.Instance.GetBaseStats(character.Job, character.Level).PriceSPStone); //sell Price
                character.Client.SendPacket(packet);
            }
        }

        private static void SendSellItems(ZoneCharacter character, string MobName)
        {
            using(var packet = new Packet(SH15Type.SellItems))
            {
                packet.WriteShort(1); //ItemsSellCount
                packet.WriteShort(0); //unk ~Randome Number
                packet.WriteByte(0);
        ...
        [PacketHandler(CH8Type.NpcOpenWindow)]
        public static void NpcAnswerHandler(ZoneClient client, Packet packet)
        {
            byte answer;
