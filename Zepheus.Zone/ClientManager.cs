...
namespace Zepheus.Zone
{
    ...
    public sealed class ClientManager
    {
        ...
        private ConcurrentDictionary<ushort, ZoneClient> clientsByObj = new ConcurrentDictionary<ushort, ZoneClient>();
        ...
        public ZoneClient GetClientByObj(ushort Obj)
        {
            Log.WriteLine(LogLevel.Debug, "Count of Character Objects ID: {0}", clientsByObj.Count);

            ZoneClient client;
            if (clientsByObj.TryGetValue(Obj, out client))
            {
                return client;
            }
            else return null;
        ...
        public bool AddClient(ZoneClient client)
        {
            ...
            {
                if (!clientsByName.TryAdd(client.Character.Name, client) || !clientsByObj.TryAdd(client.Character.MapObjectID, client))
                {
                    Log.WriteLine(LogLevel.Warn, "Could not add client to list!");
                    return false;
                }
            }
            Log.WriteLine(LogLevel.Debug, "{0} added to Zone", client.Character.Name);
            return true;
        }
        ...
        public void RemoveClient(ZoneClient client)
        {
            ...
            ZoneClient deleted;
            clientsByObj.TryRemove(client.Character.MapObjectID, out deleted);
