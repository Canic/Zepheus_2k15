...
namespace Zepheus.World.Handlers
{
    public sealed class Handler28
    {
        ...
        public static void SendPartyList(WorldClient client)
        {
            if (Program.Entity.Parties.Where(c => c.CharNo == client.Character.ID).Count() == 1)
            {
                Party party = Program.Entity.Parties.First(c => c.CharNo == client.Character.ID);
                Handler14.SendPartyList(party.PartyNo);
            }
        }
    }
}
